...
def test_refreshing_reissues_the_refresh_cookie(alice: TestClient):
    ...
    Asserted on the Set-Cookie header rather than on the token VALUE changing:
    `exp` has second resolution, so a refresh within the same second mints a
    byte-identical token. That also means this is re-issuance, NOT rotation —
    the previous refresh token stays valid until it expires. Real rotation needs
    server-side token state, which is not built (see the roadmap).
    """
